...
public class DummyLocalizer implements EstimatorInterface {
	...
	public double getCurrentProb( int x, int y) {
		return F[rows*x*head+head*y]*4;//O_diags.get(rows*x+y)[head*rows*trueRow+head*trueCol+trueHead];
	}
	public void update() {
        // gå ett steg enl T??
        takeStep();

        System.out.println(percRow + "  " + percCol + " l: " + F.length);
        ...
        System.out.println(percRow + "  " + percCol);
        // räkna ut nya F
        double[] O_diag;
        if(percRow == -1){
            O_diag = Onothing;
        ...
        double[] newF = new double[rows*cols];
        for(int i = 0;i<newF.length;i++){
            newF[i] = F[4*i];
        }
        F = Arrays.copyOf(newF,newF.length);*/
	}
	private void updateReading(){
        percRow = -1;
        percCol = -1;
        double[] nd = Arrays.copyOf(F,F.length);
        for(double d :nd){
            System.out.print(d + "  ");
        }
        int ans;
